...
/** Every 16th→15th period from `anchorISO` (a 16th) through the one containing `now`, oldest first. */
export function periodsSince(anchorISO, now = new Date()) {
  const out = [];
  let p = periodFor(new Date(anchorISO + 'T00:00:00Z'));
  const last = periodFor(now);
  while (p.start <= last.start) {
    out.push(p);
    const next = new Date(p.end + 'T00:00:00Z'); next.setUTCDate(next.getUTCDate() + 1);
    p = periodFor(next);
  }
  return out;
}
// History is reported from the Dec 16 2025 period onward (Edd, 2026-09-01).
export const HISTORY_ANCHOR = '2025-12-16';
