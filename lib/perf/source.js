...
export function parseFlight(text) {
  ...
  // Accounts are labelled. Scanning back from the first "serviceTier" used to
  // work, but the payload now carries a tracker-log array that also mentions
  // the field, so that scan landed on a 2-element array and every account was
  // lost. Take the labelled array, and prefer the longest if the key repeats.
  let accounts = null;
  for (let i = text.indexOf('"accounts":['); i >= 0; i = text.indexOf('"accounts":[', i + 1)) {
    const a = grabArray(text, i + '"accounts":'.length);
    if (!accounts || a.length > accounts.length) accounts = a;
  }
  if (!accounts || !accounts.length) throw new Error('flight payload has no account records');
...
// The dashboard is the only source these numbers ever come from, so its base
// URL is a default rather than a required setting. Requiring it cost a
// production deploy: sourceKind() reported rsc_stopgap on the cookie alone,
// then the pull died on fetch("undefined/") because CAMPAIGN_BASE_URL was
// never set. Set the env var only to point at somewhere else.
const CAMPAIGN_BASE = () => process.env.CAMPAIGN_BASE_URL || 'https://martal-campaign-dashboard.vercel.app';
...
async function fromRscStopgap() {
  ...
  if (res.status !== 200) throw new Error(`campaign dashboard returned ${res.status} for ${CAMPAIGN_BASE()}; the session cookie has probably expired, reset CAMPAIGN_SESSION_COOKIE`);
