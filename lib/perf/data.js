...
import { latestSnapshotMeta, snapshotPayload } from './cache';
...
import { companyTotals, companySeries, repTable, repHistory, churn, quality, visibleRepRows, visibleRepHistory } from './aggregate';

import { bundledSeed } from './seed';

export const loadDataset = cache(async () => {
  const snap = await latestSnapshotMeta();
  if (!snap) {
  ...
  const payload = await snapshotPayload(snap.id);
  return { ...payload, pulledAt: snap.pulled_at, source: snap.source, isSample: false, leadCount: snap.lead_count };
});
...
export async function repsView(searchParams) {
  ...
  // SOMs are counted in the company totals below but do not appear on the
  // rep dashboard at all (Edd, 2026-09-02).
  const table = visibleRepRows(repTable(ds.leads, w));
  const prevTable = visibleRepRows(repTable(ds.leads, prevCut));
  ...
  return { ds, w, prev, prevCut, inProgress, daysIn, periods, table: rows, totals, history: visibleRepHistory(repHistory(ds.leads, periods)) };
}
