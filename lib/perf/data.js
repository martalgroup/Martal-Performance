...
import { recentPeriods, preset, periodFor, monthFor, lastCompleteMonth, recentMonths } from './periods';
...
export async function churnView(searchParams) {
  ...
  return {
    ds, w, inProgress,
    churn: churn(ds.accounts, w),
    open: { w: open, ...churn(ds.accounts, open), dayOfMonth: new Date().getUTCDate() },
    series: recentMonths(6).map((p) => ({ ...p, ...churn(ds.accounts, p) })),
  };
}
