 ...
 */
export function churn(accounts, w) {
  const churnedIn = accounts.filter((a) => inWindow(a.churnDateISO || a.endISO, w));
  const startedIn = accounts.filter((a) => inWindow(a.startISO, w));
  ...
  return {
    ...
    churned: churnedIn.map((a) => ({ name: a.name, tier: a.serviceTier, mrr: a.monthlyRevenue, start: a.startISO, end: a.churnDateISO || a.endISO, owner: a.dealOwner, soms: a.soms || [] }))
