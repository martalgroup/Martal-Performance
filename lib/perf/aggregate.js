...
// SOMs whose booked meetings count for the company but who are not ranked
// against the SDRs (Edd, 2026-09-02). Their rows still appear, unranked.
export const NON_RANKED = new Set(['Angela Hamilton', 'Paige Givinsky']);
/** Rep rows / history as shown on the dashboard: SOMs are removed entirely.
 *  Company totals are computed from all leads and are unaffected. */
export const visibleRepRows = (rows) => rows.filter((r) => !NON_RANKED.has(r.rep));
export const visibleRepHistory = (hist) => Object.fromEntries(Object.entries(hist).filter(([rep]) => !NON_RANKED.has(rep)));
// One-off spelling in the source data; fold it onto the real name.
const REP_ALIASES = { 'Angela Hamlton': 'Angela Hamilton' };   // one-off spelling in the source
const canonRep = (r) => {
  const s = String(r || '').trim();
  if (!s) return UNATTRIBUTED;
  if (REP_ALIASES[s]) return REP_ALIASES[s];
...
export function repTable(leads, w) {
  ...
  return [...by.values()]
    .map((r) => ({ ...r, accounts: r.accounts.size, ranked: r.rep !== UNATTRIBUTED && !NON_RANKED.has(r.rep) }))
